...
std::vector<std::vector<BigFloat>> compute_penalty_matrix(
  ...
  for (int i = pods.size() - 3; i >= 0; --i) {
    for (int j = i + 1; j < pods.size(); ++j) {
      BigFloat val = 0;
      if (i == prevI) {
        val += penalty_matrix[prevI][prevJ] *
               std::max(pods[prevI].player_ids.size(),
                        pods[prevJ].player_ids.size());
      }
      for (int k = i + 1; k < pods.size() - 1; ++k) {
        val +=
            penalty_matrix[k][pods.size() - 1] *
            std::max(pods[k].player_ids.size(), pods.back().player_ids.size());
      }
    ...
    for (int j = i + 1; j < player_ids.size(); ++j) {
      ...
      auto penalty =
          penalty_matrix[std::min(pod1.pod_rank - 1, pod2.pod_rank - 1)]
                        [std::max(pod1.pod_rank - 1, pod2.pod_rank - 1)];
...
std::vector<Match> create_matches_mcpm(const std::vector<Player>& players,
  ...
  auto matching = create_graph_and_compute_matching(player_id_to_player, pods,
                                                    player_id_to_pod, players);
