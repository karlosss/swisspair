...
static void sort_matches(std::vector<Match> &matches) {
  std::sort(matches.begin(), matches.end(),
            [](const Match &i, const Match &j) { return i < j; });
}

static std::vector<Match> player_ids_pairs_to_matches(
    const std::vector<std::pair<std::string, std::string>> &id_pairs,
  ...
  for (const auto &pair : id_pairs) {
    if (pair.first == BYE_PLAYER_ID) {
      ...
      matches.push_back(Match(player_id_to_player.at(pair.first),
                              player_id_to_player.at(pair.second)));
    }
  }
